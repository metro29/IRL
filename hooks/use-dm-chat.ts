...
function rowFromPayload(
  ...
  return {
    ...
    conversation_id: newRow.conversation_id as string,
    sender_id: newRow.sender_id as string,
...
export function useDmChat(
  ...
  for (const m of messages) {
    authorCacheRef.current.set(m.sender_id, m.author);
  }
  ...
  const appendMessage = useCallback((msg: DmMessageWithAuthor) => {
    setMessages((prev) => {
      if (prev.some((m) => m.id === msg.id)) return prev;
      authorCacheRef.current.set(msg.sender_id, msg.author);
      return [...prev, msg];
    });
  }, []);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          void resolveAuthor(newRow.sender_id as string).then((author) => {
